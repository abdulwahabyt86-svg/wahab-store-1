...
/* =========================================================
   ...
   =========================================================
   Products are stored on the Flask server so every visitor
   sees the same catalog. localStorage is kept only as a
   fallback/cache for the shopping UI.
   ========================================================= */
...
async function loadProductsFromServer() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error("Could not load products from server.");
        }
        ...
        console.warn(
            "Server products unavailable; using local cache.",
            error
        );
...
/* Refresh the shared catalog periodically so an already-open store page
   can see products added by the admin from another device/browser. */
async function refreshSharedProducts() {
...
document.addEventListener(
    ...
    async function() {
        ...
        /* Load the shared product catalog from MySQL through Flask first. */
        await refreshSharedProducts();

        /* Check for new admin products every 15 seconds. */
        setInterval(refreshSharedProducts, 15000);
